...
try:
    ...
    with xr.open_dataset(file_path, mask_and_scale=True) as ds:
        ...
        # --- Time Slicing ---
        if 'valid_time' in var_data_u.dims and 'valid_time' in var_data_v.dims:
            if time_index_to_plot >= len(ds['valid_time']):
                 raise IndexError(f"time_index_to_plot={time_index_to_plot} out of bounds. Time size: {len(ds['valid_time'])}.")

            data_slice_u = var_data_u.isel(valid_time=time_index_to_plot)
            data_slice_v = var_data_v.isel(valid_time=time_index_to_plot)

            # Get time for title
            time_val = data_slice_u['valid_time']
            try:
        ...
        elif 'valid_time' not in var_data_u.dims and 'valid_time' not in var_data_v.dims:
             print("Warning: Wind components do not seem to have a 'valid_time' dimension. Plotting as is.")
